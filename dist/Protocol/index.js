...
var Protocol = /** @class */ (function () {
    ...
    Protocol.prototype.add = function (conn) {
        ...
        conn.on("data", function (data) {
            ...
            console.log("recieved", type, _this.listeners[type]);
            if (typeof _this.listeners[type] == typeof (function () { })) {
                var buf = new Uint8Array(data.slice(1));
                console.log("called", PackageTypes_1.PackageTypes[type], buf);
    ...
    Protocol.prototype.Build = function (playerId, uid, buildingType, targetX, targetY) {
        this.send(PackageTypes_1.PackageTypes.Build +
    ...
    Protocol.prototype.Customize = function (Customization) {
        this.send(PackageTypes_1.PackageTypes.Customize + Protobuf.encoder.encodeCustomize(Customization));
    };
    Protocol.prototype.EntityMove = function (uid, x, y, z, yaw, pitch, vx, vy, vz) {
        this.send(PackageTypes_1.PackageTypes.EntityMove +
    ...
    Protocol.prototype.Message = function (content) {
        this.send(PackageTypes_1.PackageTypes.Message + Protobuf.encoder.encodeMessage({ content: content }));
    };
    Protocol.prototype.MoveUnit = function (uid, targetX, targetY) {
        this.send(PackageTypes_1.PackageTypes.MoveUnit +
            Protobuf.encoder.encodeMoveUnit({ uid: uid, targetX: targetX, targetY: targetY }));
    };
    ...
    Protocol.prototype.SetAnimation = function (uid, animType) {
        this.send(PackageTypes_1.PackageTypes.SetAnimation +
            Protobuf.encoder.encodeSetAnimation({ uid: uid, animType: animType }));
    };
    Protocol.prototype.TryAttack = function (sourceUid, targetUid) {
        this.send(PackageTypes_1.PackageTypes.TryAttack +
            Protobuf.encoder.encodeTryAttack({ sourceUid: sourceUid, targetUid: targetUid }));
    };
    Protocol.prototype.TryBuild = function (x, y, buildingType) {
        this.send(PackageTypes_1.PackageTypes.TryBuild +
            Protobuf.encoder.encodeTryBuild({ x: x, y: y, buildingType: buildingType }));
    };
    Protocol.prototype.TryCustomize = function (Customization) {
        this.send(PackageTypes_1.PackageTypes.TryCustomize +
            Protobuf.encoder.encodeTryCustomize(Customization));
    };
    Protocol.prototype.TryDefend = function (sourceUid, targetX, targetY) {
        this.send(PackageTypes_1.PackageTypes.TryDefend +
            Protobuf.encoder.encodeTryDefend(sourceUid, targetX, targetY));
    };
    Protocol.prototype.TryJoinLobby = function (username) {
        this.send(PackageTypes_1.PackageTypes.TryJoinLobby +
            Protobuf.encoder.encodeTryJoinLobby({ username: username }));
    };
    Protocol.prototype.TrySetPolicy = function (policyId) {
        this.send(PackageTypes_1.PackageTypes.TrySetPolicy +
            Protobuf.encoder.encodeTrySetPolicy({ policyId: policyId }));
    };
    Protocol.prototype.TryStartGame = function () {
        this.send(PackageTypes_1.PackageTypes.TryStartGame + Protobuf.encoder.encodeTryStartGame());
    };
    Protocol.prototype.UpdateHealth = function (uid, hp) {
        this.send(PackageTypes_1.PackageTypes.UpdateHealth +
            Protobuf.encoder.encodeUpdateHealth({ uid: uid, hp: hp }));
    };
    Protocol.prototype.UpdateLobby = function (playerId, username) {
        this.send(PackageTypes_1.PackageTypes.UpdateLobby +
            Protobuf.encoder.encodeUpdateLobby({ playerId: playerId, username: username }));
    };
    return Protocol;
}());
